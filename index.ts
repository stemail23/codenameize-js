...
function parseOptions(options: Options | string | number): Options {
	...
	response.maxItemChars = options && typeof options.maxItemChars === 'number' && options.maxItemChars > 0 ? Math.max(3, options.maxItemChars) : 0;
	...
	if (options && Array.isArray(options.particles)) {
		...
		// Classic mode
		response.particles = options && typeof options.adjectiveCount === 'number' ? new Array(options.adjectiveCount).fill('adjective') : ['adjective'];
	...
	response.hashAlgorithm = options && typeof options.hashAlgorithm === 'string' ? options.hashAlgorithm : 'md5';
	response.separator = options && typeof options.separator === 'string' ? options.separator : '-';
...
function codenamize(options: Options | string | number): string {
	...
	if (useOptions.capitalize) {
		particles = particles.map((particle) => particle.charAt(0).toUpperCase() + particle.slice(1));
	}

	return particles.join(useOptions.separator);
}

codenamize.use = function (particles: Record<string, string[]>) {
